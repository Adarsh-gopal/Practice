...
class report_account_general_ledger_cost(models.AbstractModel):
    _name = "account.general.ledger.cost"
    _description = "Dimension Analysis"
    ...
    def _get_templates(self):
        templates = super(report_account_general_ledger_cost, self)._get_templates()
        templates['line_template'] = 'account_cost_center.line_template_general_ledger_report_cost'
        return templates
            ...
            sql = """WITH account_move_line AS (
              ...
              (
               ...
               SELECT aml.id, ref.date, aml.name,
                 ...
                 aml.amount_residual,aml.cost_center_id, aml.partner_id, aml.reconciled, aml.company_id, aml.company_currency_id, aml.amount_currency, aml.balance, aml.user_type_id, aml.analytic_account_id
    ...
    def _do_query_group_by_account(self, options, line_id):
        ...
        company = self.env['res.company'].browse(self._context.get('company_id')) or self.env.user.company_id.id
        date = self._context.get('date_to') or fields.Date.today()
            ...
            if 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                for line in amls:
                    ...
                    if len(name) > 35 and not self.env.context.get('no_format') and not self.env.context.get('print_mode'):
                        name = name[:32] + "..."
                    cost_center_name = line.cost_center_id.name
                    ...
                    if partner_name and len(partner_name) > 35  and not self.env.context.get('no_format') and not self.env.context.get('print_mode'):
                        ...
                        cost_center_name = cost_center_name[:32] + "..."
                    caret_type = 'account.move'
                    if line.move_id:
                        caret_type = 'account.invoice.in' if line.move_id.type in ('in_refund', 'in_invoice') else 'account.invoice.out'
                    elif line.payment_id:
                        caret_type = 'account.payment'
                    #changes made. instead on name , cost_center_name is placed
                    columns = [{'name': v} for v in [format_date(self.env, line.date), cost_center_name, partner_name, currency,
