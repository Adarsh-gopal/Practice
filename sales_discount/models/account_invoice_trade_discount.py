...
class AccountInvoiceDiscount(models.Model):
    _name = "account.invoice.trade.discount"
    _description = "Invoice Discount"
    _order = 'sequence'

    @api.depends('invoice_trade_discount_id.invoice_line_ids')
    def _compute_base_amount_trade_discount(self):
        discount_grouped = {}
        for invoice in self.mapped('invoice_trade_discount_id'):
            discount_grouped[invoice.id] = invoice.get_trade_discount_values()
        for discount_line in self:
            discount_line.base = 0.0
            if discount_line.discount_id:
                key = discount_line.discount_id.get_grouping_key_trade_discount({
    ...
    invoice_trade_discount_id = fields.Many2one('account.move', string='Invoice Discount', ondelete='cascade', index=True)
    ...
    amount_total = fields.Monetary(string="Amount", compute='_compute_amount_total_trade_discount')
    ...
    currency_id = fields.Many2one('res.currency', related='invoice_trade_discount_id.currency_id', store=True, readonly=True)
    base = fields.Monetary(string='Base', compute='_compute_base_amount_trade_discount', store=True)
    ...
    def _compute_amount_total_trade_discount(self):
        for tax_line in self:
            tax_line.amount_total = -tax_line.amount + tax_line.amount_rounding

#this module is for storing and displaying the total sum of trade discounts in invoice line and to pass to accounts