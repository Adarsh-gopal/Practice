...
class StockPicking(models.Model):
    ...
    z_analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account', help="The analytic account related to a sales order."
        ,compute="check_analytic_account",store=True)
...
class StockMove(models.Model):
    ...
    z_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic Tags',compute='get_Analytic_account',store=True)
    ...
    # Fetching the Analytic Tags from sale and Purchase Orders
    @api.depends('purchase_line_id','sale_line_id')
    def get_Analytic_account(self):
        # pdb.set_trace()
        for l in self:
            if l.sale_line_id:
                l.z_analytic_tag_ids = [(4, analytic_tag.id, None) for analytic_tag in l.sale_line_id.analytic_tag_ids]
            elif l.purchase_line_id:
                l.z_analytic_tag_ids = [(4, analytic_tag.id, None) for analytic_tag in l.purchase_line_id.analytic_tag_ids]
            else:
...
class StockMoveLine(models.Model):
    ...
    def compute_analytical_tags(self):
        for lines in self:
            if lines.move_id:
                lines.z_analytic_tag_ids = [(4, analytic_tag.id, None) for analytic_tag in lines.move_id.z_analytic_tag_ids]
