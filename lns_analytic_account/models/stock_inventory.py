...
class Stockquant(models.Model):
    ...
    z_analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic Tags',store=True,readonly='0',compute='get_analytic_tags')

    # Get the Analytic Tags from the lot number
    @api.depends('lot_id')
    def get_analytic_tags(self):
        for l in self:
            # print(l.lot_id)
            if l.lot_id:
                l.z_analytic_tag_ids = [(4, analytic_tag.id, None) for analytic_tag in l.lot_id.z_analytic_tag_ids]
            else:
