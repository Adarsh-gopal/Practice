...
_STATES = [
  ...
  ('to_approve', 'Open'),
  ('close', 'Close'),
  ('level_a_approved', 'Second Approval'),
  ('approved', 'Approved'),
  ('rejected', 'Rejected')
]
...
class PurchaseRequestLine(models.Model):
  ...
  def _compute_is_editable(self):
      for rec in self:
          if rec.request_id.state in ['close']:
              rec.is_editable = False
          else:
              rec.is_editable = True
  ...
  ('draft', 'Draft'),
  ('to_approve', 'Open'),
  ('close', 'Close'),],string='Request state',
                                   readonly=True,
  ...
  states = fields.Selection([('approval1','Approved'),('approval2','Confirmed'),('open','Open'),('reject','Reject')])
  categ_types = fields.Selection([('products','Products'),('services','Services'),('assets','Assets'),('charge','Charges')],'Category',default='products')
  purchase_type = fields.Many2one('res.approval',string = 'Purchase Request Type',store = True,related='request_id.purchase_type')
      ...
      for rec in self.request_id:
        if rec.z_login_id:
          if rec.state == 'to_approve':
            ...
            s = False
            for lvl in var:
              approve1=[x.z_purchase_request_approve.id for x in lvl.z_order_line_id]
                #pdb.set_trace()
              if rec.z_login_id.id in approve1 and rec.purchase_type.validation_type == 'first_level':
                self.states = 'approval2'
                s = True
                break
              elif rec.z_login_id.id in approve1 and rec.purchase_type.validation_type == 'second_level':
                self.states = 'approval1'
                s = True
                break
            if s == True:
              break
      ...
      for rec in self.request_id:
        if rec.z_login_id:
          if rec.state == 'to_approve':
            ...
            for lvl in var:
              approve1=[x.z_purchase_request_approve.id for x in lvl.z_order_line_id]
              approve2=[x.z_purchase_request_approve2.id for x in lvl.z_order_line2_id]
              # for lvl_chk in lvl.z_order_line2_id:
              #   for lvls_chk in lvl.z_order_line_id:
              #     pdb.set_trace()
              if rec.z_login_id.id in approve2:
                self.states = 'approval2'
                s = True
                break
              for l in rec.purchase_type.z_order_line_id:
                if not l.z_purchase_request_approve:
                  raise Warning('Kindly check First Level Approval.')
            if s == True:
              break
          if not s:
            raise Warning('You are not Authorized to Approve.')
...
class PurchaseOrder(models.Model):
  ...
  purchase_request_id = fields.Many2one('purchase.request', string='Purchase Request' , domain ="[('z_check_status','=','open')]")
  ...
  def purchase_order_change(self):
      ...
      for line in self.purchase_request_id.line_ids - self.order_line.mapped('purchase_request_line'):
        if not line.check_boolean and line.states == 'approval2':
  ...
  def button_confirm(self):
      for order in self:
          if order.state not in ['draft', 'sent','to_approve','approved']:
              continue
