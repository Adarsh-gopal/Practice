...
class MrpProduction(models.Model):
    ...
    def button_plan(self):
        for l in self:
            if l.bom_id.internal_ref_auto == True:
                ...
                var = self.env['stock.production.lot'].search([('name','=',l.name)])
                for line in var:
                    l.lot = line.id
                if not l.internal_reference:
                    raise UserError(_('please Enter Internal Reference'))
        return super(MrpProduction,self).button_plan()

    @api.onchange('analytic_account_id','move_raw_ids')
...
class MrpBom(models.Model):
    ...
    internal_ref_auto = fields.Boolean(string='Internal Reference Automation')
