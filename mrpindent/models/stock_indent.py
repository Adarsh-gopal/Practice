...
class IndentOrder(models.Model):
    ...
    #field added
    # maintenance_order_origin = fields.Many2many('maintenance.order','Maintenance Order No.')
    # mrp_order_origin = fields.Many2many('mrp.production','Production Order No.')
    mrp_order_ids = fields.Many2one('mrp.production','Production Order No.')
    # mrp_order_ids = fields.Many2many('mrp.production',string="MRP")
    origin_swap = fields.Boolean(string="Check if this Material Requisition is for Maintenance Order", default=False)
    origin_swap_cond = fields.Char(help="Field to compute Origin Swap. Used only for the purpose of using the function")
    #fields end
    ...
    picking_type_id = fields.Many2one('stock.picking.type', 'Operation Type',default=_default_transfers)
        ...
        if self.product_lines:
            ...
            for each_list in list_of_lists:
                ...
                for line_id in each_list:
                    line= self.env['stock.indent.order.line'].search([('id','=',line_id)])
                    ...
                    for order in self:
                        if any([order.product_lines.mapped('product_id.type')]):
                            ...
                            for each_line in each_list:
                                moves = self.env['stock.indent.order.line'].search([('id','=',each_line)])._create_stock_moves(picking)
                            self.write({'state': 'done','approve_date': datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                            in_num +=1
            # else:
